...
class OrixDoctor:
    ...
    def run_diagnostics(self) -> Dict[str, Any]:
        ...
        # 2. Check for hardcoded credentials (Severity: CRITICAL)
        found_keys = False
        found_danger = False
                ...
                for kpat in key_patterns:
                    if kpat in content:
                        ...
                        if any('"' in l or "'" in l for l in lines):
                            # Ignore assignments of config, env, none, lists or placeholder keys
                            if not any(x in line_lower for line_lower in [l.lower() for l in lines] for x in ["env", "os.get", "config", "default", "dummy", "none", "sk-proj-example", "key_patterns =", "key_patterns"]):
