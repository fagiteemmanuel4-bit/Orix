...
from orix.core.ai_providers import AIProvider, MockProvider
...
def test_ai_provider_malformed_json_repair_failure():
    ...
    provider = MockProvider({"provider": "mock", "api_key": "dummy"})

    # Mock generate to return totally malformed JSON that cannot be parsed
    provider.generate = lambda messages, **options: "This is not JSON at all!"
...
def test_ai_provider_missing_required_schema_field():
    ...
    provider.generate = lambda messages, **options: '{"other_field": "val"}'
